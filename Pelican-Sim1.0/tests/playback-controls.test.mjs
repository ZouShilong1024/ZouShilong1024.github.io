...
function environment() {
  ...
  return { doc, selectors, observers, tick: () => { const callbacks = [...frames.values()]; frames.clear(); callbacks.forEach(fn => fn()); } };
}
...
test('overview: both rows switch sources and synchronize within their own task', () => {
  const { doc, selectors, observers, tick } = environment();
  ...
  rows[0][1].readyState = 2;
  tick();
  assert.ok(rows[0].every(video => video.paused));
  assert.ok(rows[1].every(video => !video.paused), "buffering only pauses its own row");
  rows[0][1].readyState = 4;
  tick();
  assert.ok(rows[0].every(video => !video.paused));
