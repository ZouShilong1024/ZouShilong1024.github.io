import { createSceneGroup } from "./scene-sync.js";
...
export function initOverview() {
  ...
  const rows = [primaryVideos, secondaryVideos].filter(row => row.length);
  const groups = rows.map(row => createSceneGroup(row, { loadOnActivate: false }));
  const loaders = rows.map(row => createMediaLoader(row));
  ...
  const playOverviewVideos = () => {
    ...
    groups.forEach(group => group.activate());
  };
  const pauseOverviewVideos = () => groups.forEach(group => group.pause());
  ...
  const showOverviewDataset = (index, immediate = false) => {
    ...
    loaders.forEach(loader => loader.cancel());
    ...
    const markLoaded = () => {
      row.forEach((video) => { video.currentTime = 0; });
      overviewStage.classList.remove("is-changing");
      playOverviewVideos();
    };
    loaders[rowIndex].load((video) => {
