#!/usr/bin/env bash
# World-Sim 1.0 preview on 0.0.0.0:8000.
...
print_ok() {
  local extra="${1:-}"
  echo "World-Sim 1.0 preview ${extra}on ${BIND}:${PORT}"
...
start_in_tmux() {
  tmux new-session -d -s "${SESSION}" -n preview -c "${ROOT}" \
    "echo \$\$ > '${PIDFILE}'
     while true; do
       echo \"[\$(date -Is)] World-Sim 1.0 preview starting on ${BIND}:${PORT}\" | tee -a '${LOGFILE}'
...
echo "Failed to start World-Sim 1.0 preview on ${BIND}:${PORT}; see ${LOGFILE}" >&2
if tmux_session_exists; then
